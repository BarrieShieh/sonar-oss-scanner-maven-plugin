...
public class MojoExecutorTest {
  ...
  @Test
  public void executeMojoWithoutExecutionIdExecutesMojoWithAttributesInConfiguration()
      throws Exception {
    executeMojo(
        plugin(
            groupId("org.apache.maven.plugins"),
  ...
  @Test
  public void executeMojoWithoutExecutionIdExecutesMojoWithExplicitConfiguration()
      throws Exception {
  ...
  @Test
  public void executeMojoWithoutExecutionIdExecutesMojoWithExplicitConfigurationNoProject()
      throws Exception {
  ...
  @Test
  public void executeMojoWithoutExecutionIdExecutesMojoWithExplicitConfigurationIgnoreProject()
      throws Exception {
  ...
  private static class MojoExecutionIsEqual extends TypeSafeDiagnosingMatcher<MojoExecution> {
    ...
    protected boolean matchesSafely(MojoExecution mojoExecution, Description mismatchDescription) {
      ...
      matches = tryMatch("executionId", executionId, mojoExecution.getExecutionId(),
          mismatchDescription, matches);
      matches = tryMatch("mojoDescriptor", mojoDescriptor, mojoExecution.getMojoDescriptor(),
          mismatchDescription,
          matches);
      matches = tryMatch("configuration", configuration, mojoExecution.getConfiguration(),
          mismatchDescription,
          matches);
      matches = tryMatch("source", source, mojoExecution.getSource(), mismatchDescription, matches);
      matches = tryMatch("lifecyclePhase", lifecyclePhase, mojoExecution.getLifecyclePhase(),
          mismatchDescription,
          matches);
    ...
    private boolean tryMatch(String name, Matcher<?> matcher, Object item,
    ...
    private void reportMismatch(String name, Matcher<?> matcher, Object item,
        Description mismatchDescription,

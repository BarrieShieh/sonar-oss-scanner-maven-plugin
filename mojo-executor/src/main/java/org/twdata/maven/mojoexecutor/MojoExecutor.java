...
/**
 * Executes an arbitrary mojo using a fluent interface.  This is meant to be executed within the
 * context of a Maven mojo. Here is an execution that invokes the dependency plugin:
 * <pre>
 * executeMojo(
 *              plugin(
...
public class MojoExecutor {
  ...
  public static void executeMojo(Plugin plugin, String goal, Xpp3Dom configuration,
    ...
    try {
      ...
      if ((plugin.getVersion() == null || plugin.getVersion().length() == 0)
        ...
        if (pm != null) {
          for (Plugin p : pm.getPlugins()) {
            if (plugin.getGroupId().equals(p.getGroupId()) && plugin.getArtifactId()
                .equals(p.getArtifactId())) {
      ...
      PluginDescriptor pluginDescriptor = MavenCompatibilityHelper
          .loadPluginDescriptor(plugin, env, session);
  ...
  private static MojoExecution mojoExecution(MojoDescriptor mojoDescriptor, String executionId,
    ...
    if (executionId != null) {
      ...
      configuration = Xpp3DomUtils
          .mergeXpp3Dom(configuration, toXpp3Dom(mojoDescriptor.getMojoConfiguration()));
